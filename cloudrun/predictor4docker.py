...
class MyPredictor(object):
    ...
    def predict(self, instances, **kwargs):
        ...
        if kwargs.get('probabilities'):
            ...
            return {"predictions": np.concatenate((ids.values.reshape(-1, 1), probabilities), axis=1).tolist()}
        else:
            logger.info("Predicting values...")
            outputs = self._model.predict(preprocessed_inputs)
            return {"predictions": np.concatenate((ids.values.reshape(-1, 1), outputs), axis=1).tolist()}
